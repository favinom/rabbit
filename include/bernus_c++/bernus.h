 ...
 *
 * The model is also described at:
 * https://models.physiomeproject.org/e/5/bernus_wilders_zemlin_verschelde_panfilov_2002.cellml/view
 *
 * The bernus class defined here implements the Iionmodel interface and collects
 * all routines
 * that involve the (time-dependent) gating variables. Static functions, that is
 * functions that
 * do not dependent on the gating variables, are collected in bernus_functions.
 * For a single cell the Bernus model (and many other membrane models) is a
 * differential equation
 *
 * \\( v' = -I_{\rm ion}(v, w) \\\
 *  w' = f(v, w) \\)
 *
 * where \\( v \\) is the membrane potential, \\( w \\) are the state variables
 * of the membrane model
 * (''gating variables'') and both \\( I_{\rm ion} \\) and \\( f \\) are
 * provided by the model.
 * In the Bernus model, \\( I_{\rm ion} \\) is the sum of nine different ion
 * currents:
 ...
 *
 * <b>A note on physical units</b>: The ionic current return by #ionforcing is
 * in millivolt per millisecond \\( \\textrm{mV}~\\textrm{ms}^{-1} \\): The
 * constants #g_na, #g_ca etc are in
 *
 * \\( \\textrm{nS}~\\textrm{pF}^{-1} = 10^3 \\textrm{S}~\\textrm{F}^{-1} =
 * 10^3~\\textrm{s}^{-1} = \\textrm{ms}^{-1} \\)
 *
 * with S = Siemens and F = Farad, cf. Table 1 in Bernus et al. The membrane
 * potential and equilibrium potential both are in millivolt.
 */
...
public:
  ...
  //! Static factory function that instantiates a #bernus object and returns a
  //! pointer. Called by the #IionmodelFactory class.
  ...
  //! Object providing all the necessary functions to compute parameters that do
  //! not depend on the gating variables.
  ...
  double static const g_naca = 1000.0;
...
/*
 * To faciliate inlining, the ion current functions are implemented here, in the
 * header file.
 * The keyword 'inline' allow e.g. the use of the -Winline flag for gcc to issue
 * a warning if
 * the compiler was unable to actually inline the respective function.
 */

inline double
bernus::ionforcing(double V, std::vector<double> *gates)
{
  return i_na(V, gates) + i_ca(V, gates) + i_to(V, gates) + i_k(V, gates) +
         i_k1(V) + i_b_ca(V) + i_b_na(V) + i_na_k(V) + i_na_ca(V);
}
...
inline void
bernus::get_gates_dt(double V, std::vector<double> *gates,
                     std::vector<double> *gates_dt)
{
  ...
  (*gates_dt)[m_gate] = bnf.alpha_m(V) * (1.0 - (*gates)[m_gate]) -
                        bnf.beta_m(V) * (*gates)[m_gate];
  (*gates_dt)[f_gate] = bnf.alpha_f(V) * (1.0 - (*gates)[f_gate]) -
                        bnf.beta_f(V) * (*gates)[f_gate];
  (*gates_dt)[to_gate] = bnf.alpha_to(V) * (1.0 - (*gates)[to_gate]) -
                         bnf.beta_to(V) * (*gates)[to_gate];
...
inline void
bernus::rush_larsen_step(double V, double dt, std::vector<double> *gates)
{
...
// Sodium current i_Na
inline double
bernus::i_na(double V, std::vector<double> *gates)
{
  return g_na * pow((*gates)[m_gate], 3.0) * pow((*gates)[v_gate], 2.0) *
         (V - bnf.e_na);
}

// Calcium current i_Ca
inline double
bernus::i_ca(double V, std::vector<double> *gates)
{
  return g_ca * (bnf.d_inf(V)) * (*gates)[f_gate] * (bnf.f_ca(V)) *
         (V - bnf.e_ca);
}

// Transient outward current i_to
inline double
bernus::i_to(double V, std::vector<double> *gates)
{
...
inline double
bernus::i_k(double V, std::vector<double> *gates)
{
